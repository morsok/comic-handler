...
func (tx *Tx) init() {
	...
	tx.ReadingList = NewReadingListClient(tx.config)
	tx.ReleaseWeek = NewReleaseWeekClient(tx.config)
	tx.Serie = NewSerieClient(tx.config)
	tx.StoryArc = NewStoryArcClient(tx.config)
...
// applies a query, for example: Issue.QueryXXX(), the query will be executed
